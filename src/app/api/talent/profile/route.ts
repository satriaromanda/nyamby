  ...
  try {
    ...
    const updated = await prisma.talentProfile.update({
      ...
      data: {
        ...
        ...(slug !== undefined && { slug }),
        ...(bank_code !== undefined && { bankCode: bank_code }),
        ...(bank_account !== undefined && { bankAccount: bank_account }),
        ...(bank_account_name !== undefined && { bankAccountName: bank_account_name }),
      },
    });

    // Handle skills update
    if (skills && Array.isArray(skills)) {
      const currentSkillIds = profile.talentSkills.map(s => s.skillId).sort().join(',');
      const newSkillIds = skills.map(s => s.id).sort().join(',');
