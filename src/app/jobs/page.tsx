  ...
  return (
    <div className="min-h-screen bg-surface-950">
      <nav className="glass sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex gap-4">
            <Link href="/login" className="text-sm text-surface-200 hover:text-white">Masuk</Link>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <p className="text-surface-200 mb-8">Temukan project yang sesuai dengan keahlianmu.</p>
            ...
            <button
              ...
              className={`text-sm px-4 py-2 rounded-xl transition-all ${
                category === f.value
                  ...
                  : "glass text-surface-200 hover:bg-white/5"
              }`}
            >
              {f.label}
            </button>
        ...
        {loading ? (
              ...
              <div key={job.id} className="glass rounded-2xl p-6 card-hover">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-surface-200 mb-3 line-clamp-2">{job.description}</p>
                    <div className="flex flex-wrap gap-1.5 mb-3">
                      {job.required_skills.map((s, i) => (
                        <span
                          key={i}
                          className={`text-[10px] px-2 py-0.5 rounded-full ${
                            s.is_mandatory ? "bg-primary-500/15 text-primary-300" : "bg-white/5 text-surface-200"
                          }`}
                        >
          ...
          <div className="glass rounded-2xl p-16 text-center">
            ...
            <h3 className="text-xl font-bold mb-2" style={{ fontFamily: "Outfit" }}>Belum ada job aktif</h3>
            <p className="text-surface-200 text-sm">Job baru akan muncul saat client posting.</p>
          </div>
        )}
      </div>
    </div>
