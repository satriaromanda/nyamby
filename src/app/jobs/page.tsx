...
import { Briefcase, Code, Palette, Search, Banknote, Calendar, ArrowRight, Star, ChevronRight } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="flex gap-2 mb-8">
          {[
            ...
            { value: "web_dev", label: "Web Dev", Icon: Code },
            { value: "graphic_designer", label: "Design", Icon: Palette },
          ].map((f) => (
            <button
              key={f.value}
            ...
            {jobs.map((job) => (
              <div key={job.id} className="glass rounded-2xl p-6 card-hover">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-3">
                      {job.required_skills.map((s, i) => (
                        ...
                        >
                          {s.name}
                          {s.is_mandatory && <Star className="w-2.5 h-2.5" />}
                        </span>
                      ))}
                    </div>
                  ...
                  >
                    Lamar <ChevronRight className="w-3 h-3" />
                  </Link>
                </div>
              </div>
            ))}
