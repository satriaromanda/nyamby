...
export function TalentSearch({ isDashboard = false }: { isDashboard?: boolean }) {
  const [lang] = useLang();
  const t = copy[lang];
  ...
  const content = (
    <main role="main" className={`max-w-7xl mx-auto px-6 pb-12 ${isDashboard ? 'pt-8' : 'pt-28'}`}>
      ...
      <div className="flex flex-col gap-4 mb-8">
        ...
        {/* Top: Categories */}
        <div className="flex overflow-x-auto pb-2 lg:pb-0 hide-scrollbar gap-2 w-full">
          {categories.map((f) => (
            <button
              key={f.value}
      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-5">
          {talents.map((talent) => {
            ...
            const talentHref = isDashboard ? `/client/talents/${talent.id}` : `/talents/${talent.id}`;

            return (
              <Link key={talent.id} href={talentHref} className="card card-hover p-6 block">
